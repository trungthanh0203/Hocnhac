...
export default function CodesAdmin() {
  ...
  const [codeType, setCodeType] = useState('level') // 'level' | 'module'
  ...
  function toggleModule(name) {
    setSelectedModuleNames(prev => prev.includes(name) ? prev.filter(x => x !== name) : [...prev, name])
  }

  async function createCode() {
    if (codeType === 'level' && !selectAll && selectedLevelIds.length === 0) {
      setMsg({ type: 'error', text: 'Chọn ít nhất 1 cấp, hoặc tick "Mở tất cả 9 cấp".' })
      return
    }
    if (codeType === 'module' && selectedModuleNames.length === 0) {
      setMsg({ type: 'error', text: 'Chọn ít nhất 1 module.' })
      return
    }
    ...
    const payload = {
      ...
      note: newNote || null,
      unlock_level_ids: codeType === 'level' ? (selectAll ? null : selectedLevelIds) : null,
      unlock_module_names: codeType === 'module' ? selectedModuleNames : null,
    }
    ...
    if (!error) { setNewNote(''); setSelectedLevelIds([]); setSelectedModuleNames([]); reload() }
  }
  ...
  return (
    <div className="admin-panel">
      ...
      <div className="admin-row">
        <div className="admin-field">
          ...
          <select value={codeType} onChange={e => setCodeType(e.target.value)}>
            <option value="level">Theo cấp (mở toàn bộ 1 hay nhiều cấp)</option>
            <option value="module">Theo module (mở 1 kỹ năng xuyên suốt mọi cấp)</option>
          </select>
        </div>
      </div>
      ...
      {codeType === 'level' ? (
        ...
        <div className="admin-row" style={{ alignItems: 'flex-start' }}>
          <div className="admin-field">
            <label>Mở module nào (xuyên suốt cả 9 cấp)</label>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 14px', padding: '8px 12px', border: '1.5px solid var(--line)', borderRadius: 8, minWidth: 320 }}>
              {MODULE_NAMES.map(name => (
                <label key={name} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12.5 }}>
                  <input type="checkbox" checked={selectedModuleNames.includes(name)} onChange={() => toggleModule(name)} />
                  {name}
                </label>
              ))}
            </div>
      ...
      <table className="admin-table">
        <thead><tr><th>Mã</th><th>Loại</th><th>Mở</th><th>Hạn dùng</th><th>Trạng thái</th><th>Ghi chú</th><th></th></tr></thead>
        <tbody>
          {codes.map(c => {
            const s = statusOf(c)
            return (
              <tr key={c.id}>
                ...
                <td>{c.code_type === 'module' ? 'Module' : 'Cấp'}</td>
                <td>{c.code_type === 'module' ? (c.unlock_module_names || []).join(', ') : levelNames(c.unlock_level_ids)}</td>
